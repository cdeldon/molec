...
molec_force_calculation arg_get_force_routine(const char* key)
{
    ...
    printf("Using reference N2 implementation\n");
    return &molec_force_N2_refrence;
}
...
molec_force_integration arg_get_integration_routine(const char* key)
{
    ...
    printf("Using reference leap-frong implementation\n");
    return &molec_integrator_leapfrog_refrence;
}
...
int main(int argc, char** argv)
{
    ...
    struct arg_str  *arg_force_routine = arg_str0("f","force","string","force routine specification: "
                                                  "[ N2 | cell | cellfor | cellswap | celldp1 | celldp2 ]");
    // integrator routine can appear at most once --> arg_str0
    struct arg_str  *arg_integrator_routine = arg_str0("i","integrator","string","integrator routine specification: "
                                                  "[ lf | lf2 ]");
    //parameter can appear at most once --> arg_file0
    struct arg_file  *arg_parameters = arg_file0("c","config","string","path to configuration file");
   ...
   molec_force_integration force_integration = arg_get_integration_routine(arg_integrator_routine->sval[0]);
